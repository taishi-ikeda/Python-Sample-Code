...
class heap_sort(sort_base.sort_base):
    ...
    def __init__(
        self, verbose: bool, maxsize: int, func: sort_base.order_func_base
    ) -> None:
    ...
    def push_heap(self, x: int) -> bool:
        ...
        if self._verbose:
            print("   ", self._heap)
        while j > 0 and self._func.get(x) > self._func.get(self._heap[j - 1]):
    ...
    def delete_max_heap(self) -> [bool, int]:
        ...
        while i * 2 <= self._current_size:
            j = i * 2
            if i * 2 + 1 <= self._current_size and self._func.get(
                self._heap[i * 2 - 1]
            ) < self._func.get(self._heap[i * 2]):
                j = i * 2 + 1
            if self._func.get(self._heap[i - 1]) >= self._func.get(self._heap[j - 1]):
                break
            else:
